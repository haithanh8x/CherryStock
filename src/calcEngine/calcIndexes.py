...
def calculate_VNINDEX_NOT_VIN(connection=None, repository=None):
    import pandas as pd
    ...
    conn = connection or DuckDBManager.get_connection()
    ...
    try:
        ...
        if owns_connection:
            DuckDBManager.close_connection(conn)
    ...
    if repository is not None:
        repository.replace_index_series(index_name=index_name, dataframe=result_df, table_name=table_name)
        return
