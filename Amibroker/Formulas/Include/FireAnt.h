...
function GetWL()
{
	...
	do {
		...
		WatchlistStr = CategoryGetSymbols( categoryWatchlist, i++);
		if ( WatchlistStr != "" )
			nl = 1+ StrCount( WatchlistStr, ",");
		else
			nl = 0;
		
		
		
		if (listname != "" AND nl>0) {
			if (liststr != "")
				liststr = liststr + "|" + listname;
			else
				liststr = listname;
			
			//printf("liststr: %s, number: %f, WatchlistStr: %s\n", liststr, nl, WatchlistStr);
		}
