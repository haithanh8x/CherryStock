...
def _load_records(connection, run_id: str) -> list[SourceEffectivenessRecord]:
    rows = connection.execute(
        ...
        SELECT
            ...
            "HorizonBars", "AttributionMode", "LineageEventCount",
    ...
    for row in rows:
        result.append(
            SourceEffectivenessRecord(
                ...
                lineage_event_count=int(row[7] or 0),
                validation_event_count=int(row[8] or 0),
                test_event_count=int(row[9] or 0),
                touch_rate=float(row[10]) if row[10] is not None else None,
                hold_rate_given_touch=float(row[11]) if row[11] is not None else None,
                break_rate_given_touch=float(row[12]) if row[12] is not None else None,
                retest_rate_given_break=float(row[13]) if row[13] is not None else None,
                directional_edge_pct=float(row[14]) if row[14] is not None else None,
                validation_quality=float(row[15] or 0.0),
                test_quality=float(row[16] or 0.0),
                validation_marginal_lift=float(row[17] or 0.0),
                test_marginal_lift=float(row[18] or 0.0),
                temporal_stability=float(row[19] or 0.0),
                regime_stability=float(row[20]) if row[20] is not None else None,
                complexity_delta=float(row[21] or 0.0),
                effectiveness_score=float(row[22] or 0.0),
                recommendation=str(row[23]),
                evidence=json.loads(row[24] or "{}"),
            )
        )
    if not result:
