...
View hiện có **19 columns**, được ghép từ `cal_smart_money_ticker_score`, metadata model và phần pivot `NormalizedValue` từ `cal_smart_money_factor_values`. View chỉ là public/read contract; nó không tính lại model. Calculation ownership vẫn nằm trong `src/calcEngine/smartMoneyScore.py` và các bảng `cal_*` nội bộ.

Điểm cần hiểu trước: các score như `FreshFlowScore`, `RelativeLiquidityScore`, `RelativeStrengthScore`, `AccumulationScore`, `TrendScore`, `DistributionScore` chủ yếu được **cross-sectional percentile theo toàn bộ universe cùng ngày**, tức giá trị raw cao hơn trong universe sẽ có percentile cao hơn và các giá trị đứng đầu thị trường tiến gần 100. `AccumulationMemoryScore`, `SupplyLockScore`, `LimitUpScore` là score tính trực tiếp/composite và không percentile lại. Normalization chỉ chạy khi cùng ngày có tối thiểu 5 giá trị hợp lệ.

### Nguồn dữ liệu chính

| Source | Dùng cho |
|---|---|
| `main.vw_Ticker_OHLC_D` | OHLCV, `TradingValue`, provenance/quality của TradingValue |
| `main.raw_index_eod` (`VNINDEX`) | benchmark return cho RS5/20/60 |
| `main.vw_Ticker_indicators` + `main.vw_Indicator_config` | `MA20_D`, `MA50_D`, `OBV_D`, `AD_D` |
| `main.vw_stock_market_limit_eod` khi available | `LimitUp`, `LimitUpStreak`, market-limit quality đáng tin cậy |
| `main.dim_smart_money_model/config/state_weight/factor` | model identity, threshold, factor metadata và state-specific weights |
| `main.cal_smart_money_factor_values` | long-form raw/normalized factor values |
| `main.cal_smart_money_ticker_score` | final score, state, confidence và data-quality fields |

### Quy ước normalization

Các column sau được normalize từ raw feature bằng **same-date cross-sectional percentile** trên eligible active universe:
...
```text
PercentileScore(Ticker, Date) = percentile rank của RawValue
                                trong eligible universe cùng Date
                                × 100
```

Nếu cùng ngày có ít hơn 5 giá trị hợp lệ thì percentile score là `NULL`.

Các column sau đã là bounded/composite score và **không percentile lại**:
...
Vì vậy `RelativeStrengthScore = 80` và `SupplyLockScore = 80` đều nằm trên thang `0..100`, nhưng không mang cùng một ý nghĩa normalization.

### Chi tiết 19 columns

| Column | Ý nghĩa | Nguồn dữ liệu chính | Công thức / logic V1 |
| --- | --- | --- | --- |
| `Ticker` | Mã cổ phiếu | `cal_smart_money_ticker_score` | Giữ nguyên mã chứng khoán của kết quả tính. |
| `Date` | Ngày giao dịch / ngày tính score | `cal_smart_money_ticker_score` | As-of date; toàn bộ feature của ngày T chỉ dùng dữ liệu có ngày `<= T`, không dùng future data. |
| `ModelCode` | Mã model Smart Money | `dim_smart_money_model` | V1 hiện là `SMART_MONEY_V1`. |
| `ModelVersion` | Version model | `dim_smart_money_model` | V1 hiện là `1.0.0`; dùng để reproduce/backtest đúng model version. |
| **`SmartMoneyScore`** | Điểm tổng hợp sức mạnh dòng tiền thông minh, `0..100` | Các factor score + `dim_smart_money_state_weight` + `DistributionScore` | `PositiveScore = Σ(weight × factor) / Σ(weight khả dụng)`. Sau đó `SmartMoneyScore = clip(PositiveScore - Penalty(state) × DistributionScore, 0,100)`. Penalty mặc định `0.35`; state `DISTRIBUTION` dùng `0.75`. |
| **`ConfidenceScore`** | Độ tin cậy của `SmartMoneyScore`; không phải bullish/bearish score | Coverage + history + liquidity + TradingValue quality + benchmark + indicator + market-limit evidence | `30% FactorCoverage + 15% History + 15% LiquidityAdequacy + 15% TradingValueQuality + 10% Benchmark + 10% IndicatorAvailability + 5% MarketLimit`. Tất cả quy về `0..100`. |
| **`MarketState`** | Trạng thái thị trường chính của ticker trong ngày | Các normalized factors + config thresholds | State gồm `ACCUMULATION`, `BREAKOUT`, `DEMAND_EXPANSION`, `SUPPLY_LOCK`, `MARKUP`, `DISTRIBUTION`, `LIQUIDITY_DRYUP`, `SELLING_CLIMAX`, `NEUTRAL`. `DISTRIBUTION` có precedence cao nhất V1. |
| **`FactorCoverage`** | Bao nhiêu trọng số factor cần cho state hiện tại thực sự có dữ liệu | State weight + các factor non-NULL | `AvailableWeight / TotalWeight`. Range `0..1`. Ví dụ state cần tổng weight 1.0 nhưng chỉ có factor tương ứng 0.8 weight → coverage `0.8`. Missing evidence không bị biến thành zero evidence. |
| **`DataQualityStatus`** | Trạng thái sử dụng kết quả | `SmartMoneyScore`, `ConfidenceScore`, `FactorCoverage` | `INVALID` nếu SmartMoneyScore NULL; `PASS` nếu Confidence ≥60 **và** FactorCoverage ≥0.80; còn lại `WARNING`. |
| **`FreshFlowScore`** | Dòng tiền mới đang vào kèm sức mạnh giá/ngày đóng cửa | OHLCV + TradingValue từ `vw_Ticker_OHLC_D`; VNINDEX cho `RS5` | Raw: `(CLV + 2×Return5 + 1.5×RS5) × ln(1 + max(RVAL20,0))`; sau đó percentile cùng ngày → `0..100`. |
| **`RelativeLiquidityScore`** | Thanh khoản hiện tại lớn đến đâu so với bình quân 20 phiên của chính cổ phiếu | `TradingValue` từ `vw_Ticker_OHLC_D` | `RVAL20 = TradingValue / ALV20`; `ALV20 = MA20(TradingValue)`; sau đó percentile cùng ngày. |
| **`LiquidityAccelerationScore`** | Thanh khoản ngắn hạn đang tăng tốc hay giảm so với nền 20 phiên | `TradingValue` từ `vw_Ticker_OHLC_D` | `LiquidityAccelerationRaw = ALV5 / ALV20`, trong đó `ALV5 = MA5(TradingValue)`; sau đó percentile cùng ngày. |
| **`RelativeStrengthScore`** | Sức mạnh giá cổ phiếu so với VNINDEX trên nhiều horizon | Close ticker + `raw_index_eod` VNINDEX | `RSn = Return_n(ticker) - Return_n(VNINDEX)`. Raw = `20%×RS5 + 50%×RS20 + 30%×RS60`; sau đó percentile cùng ngày. |
| **`AccumulationScore`** | Mức độ tích lũy: đóng cửa mạnh, outperform, OBV/AD cải thiện | OHLCV + VNINDEX + `OBV_D`, `AD_D` từ `vw_Ticker_indicators` | Raw = mean của các evidence khả dụng: `CLV20`, `RS20×10`, `OBVSlope20`, `ADSlope20`; cần ít nhất **2/4** evidence; sau đó percentile. `OBVSlope20=(OBV_t-OBV_t-20)/MA20(Volume)`; AD tương tự. |
| **`AccumulationMemoryScore`** | “Bộ nhớ” tích lũy, tránh việc một phiên yếu làm mất ngay tín hiệu tích lũy trước đó | `AccumulationScore` lịch sử + optional persisted incremental seed | EWMA với `MEMORY_LAMBDA=0.90`: xấp xỉ `Memory_t = 0.90×Memory_(t-1) + 0.10×AccumulationScore_t`. Không percentile lại. |
| **`SupplyLockScore`** | Đo trạng thái nguồn cung bị khóa/cạn trong khi giá và RS vẫn khỏe | Memory + CloseStrength + RS + Trend + LiquidityCompression + Distribution | Geometric composite của `AccumulationMemory`, `CloseStrength`, `RelativeStrength`, `Trend`, `LiquidityCompression`; cần ≥4/5 thành phần. Sau đó `× (1 - DistributionScore/100)`, clip `0..100`. |
| **`LimitUpScore`** | Evidence cổ phiếu tăng trần và chuỗi tăng trần | `vw_stock_market_limit_eod`: `LimitUp`, `LimitUpStreak`, quality | Khi source đáng tin: nếu `LimitUp=True`: `100 × (1 - exp(-0.55 × LimitUpStreak))`; nếu `False` → `0`; evidence không đủ tin cậy → `NULL`. |
| **`TrendScore`** | Vị trí giá so với trend MA20/MA50 | Close + `MA20_D`, `MA50_D` từ Indicator Engine | `TrendRaw = mean(Close/MA20 - 1, Close/MA50 - 1)` trên giá trị khả dụng; sau đó percentile. |
| **`DistributionScore`** | Mức độ phân phối: thanh khoản cao nhưng đóng cửa yếu, giá yếu và underperform | OHLCV + TradingValue + VNINDEX | `Participation=max(RVAL20-1,0)`. `Raw = Participation × [0.40×max(-CLV,0) + 0.35×max(-Return5,0)×10 + 0.25×max(-RS20,0)×10]`; sau đó percentile. |
...
V1 tính `Return1`, `Return5`, `Return20`, `Return60` theo từng ticker và không look-ahead.
...
```text
CLV gần +1 → Close gần High trong ngày
CLV gần  0 → Close gần giữa range
CLV gần -1 → Close gần Low trong ngày
```

Khi `High == Low`, CLV là `NULL`, không chia cho 0.
...
`RelativeLiquidityScore` trả lời **hôm nay thanh khoản bất thường đến đâu so với nền 20 phiên của chính ticker**, còn `LiquidityAccelerationScore` trả lời **5 phiên gần đây thanh khoản có đang tăng tốc so với nền 20 phiên hay không**.
...
Horizon 20 phiên có trọng số lớn nhất trong V1.

### MarketState rules — implemented V1

State assignment bắt đầu từ `NEUTRAL`; state precedence thấp được áp trước và state precedence cao overwrite sau:
...
`DISTRIBUTION` là state có precedence cao nhất trong V1. Vì vậy ticker có thể đồng thời thỏa một state bullish phía dưới nhưng kết quả cuối cùng vẫn là `DISTRIBUTION` nếu DistributionScore vượt threshold.

### Cách đọc 3 output chính

```text
SmartMoneyScore  = Dòng tiền đang mạnh đến đâu sau khi trừ Distribution penalty?
MarketState      = Dòng tiền đó đang ở pha nào?
ConfidenceScore  = Có thể tin kết luận trên đến mức nào?
```
...
→ model thấy tín hiệu tích lũy mạnh và evidence tương đối đầy đủ/đáng tin.
...
→ directional/composite signal mạnh nhưng supporting evidence còn thiếu hoặc quality thấp, ví dụ thiếu benchmark/indicator/market-limit evidence hoặc TradingValue chỉ có quality thấp hơn.

### View construction

SQL view cố tình giữ mỏng:
...
Các factor-wide column được tạo bằng conditional aggregation trên `NormalizedValue`; view không tính lại factor formula. `cal_smart_money_factor_values` long-form vẫn là internal owner của component values, tránh duplicate persisted SSOT.
