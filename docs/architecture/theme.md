...
## SmartMoney ticker interaction (2026-09-26)

All ticker links are bold and use their row's TradeAction color: BUY → positive,
HOLD → warning, SELL → negative, unknown → muted. Hover (10%), active (18%),
and focus ring use that same semantic color via per-element CSS variables.
Transitions remain 150ms, with no size/weight shift; reduced-motion disables them.
State lists use one confidence-ranked wrapping row, without MA200 headers or commas.
The data builder retains its MA200 diagnostic contract for other consumers.

CherryStock ticker/detail tooltips use the R/S tooltip appearance: theme
tooltip_background/text/border, 1px border, 4px radius, 10px padding, 14px text,
bold title, wrapped explanation and separate illustrative example. They open on
hover/focus and close on blur/Escape. R/S point hints use the same HTML layout.
TradingView's internal tooltips remain vendor-owned.
