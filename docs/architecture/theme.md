...
Shared class `cs-ticker-link` is generated by `build_nicegui_css()`: primary
text, 10% primary hover background, 18% active background, underline and pointer;
2px focus-visible outline with 2px offset. Only color/background transition
(150ms ease-out); padding, weight and size never change on hover. Top-two bold
ranking stays controlled by the SmartMoney renderer. Reduced-motion disables
the transition. All colors derive from the active theme.

Ticker and detail-field tooltips are available on hover and keyboard focus,
with meaning, units and explicitly illustrative examples. Tooltip focus is cleared
on blur/Escape. Popup colors also consume the centralized theme.
